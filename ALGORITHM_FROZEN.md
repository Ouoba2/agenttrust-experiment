...
**Date**: 2026-09-05T13:25:26.844Z  
**Status**: FROZEN FOR BLIND TEST  
**Notice**: Neither `src/evidence.ts` nor `src/reputation.ts` shall be modified during this blind evaluation.

---

## 1. Summary of Tracked Signals

1. **Counterparty Diversity & Concentration** (`detectSybilPatterns`)
   - Herfindahl-Hirschman index of counterparty shares.
   - Unique counterparties count vs. total transactions.
   - Circular transaction detection (buying and selling reciprocally).
   - Round-robin sequential patterns.

2. **Amount Uniformity & Rating Perfection** (`detectSybilPatterns`)
   - Amount Coefficient of Variation (`amount_cv`): Low variance ($CV < 0.15$ or $< 0.30$) indicates manufactured or uniform transactions.
   - Rating perfection: Ratio of 5.0 star reviews >= 95%.

3. **Temporal Dynamics** (`analyzeTemporalPatterns`)
   - Burst activity: High ratio of transactions within < 10% of average interval.
   - Interval Coefficient of Variation (`interval_cv`): Very low variance indicates robotic / cron-like scheduling.

4. **Late Fraud & Degradation** (`detectLateFraudPatterns`)
   - Comparison of historical quality (first 80%) vs. recent quality (last 20%).
   - Guard condition: Requires genuine degradation (`quality_drop > 0.15`), high-value failures (> $1,000), or lost disputes (`dispute.winner === 'buyer'`).
   - Recency-weighted failure rate with exponential half-life of 30 days.
   - Value-weighted dispute rate on high-value volumes.

5. **Network Topology & Adaptive Attacker Detection** (`detectAdaptiveAttacker`)
   - **Network Isolation Score**: 1 - (external / total connections). High isolation (> 0.80) flags closed sybil rings.
   - **Counterparty Overlap**: Density of cross-connections among the agent's counterparties. High overlap (> 0.25) flags embedded collusion cartels.
   - Rapid account growth vs. age in days.

---

## 2. Key Thresholds & Penalty Rules

- **Classification Threshold**: Overall Trust > 70.0 classifies an agent as Good / Honest. <= 70.0 classifies as Bad / Malicious.
- **Base Trust Composition**:
  - Identity Confidence (8%)
  - Delivery Reliability (15%)
  - Economic Reliability (15%)
  - Dispute Performance (12%)
  - Counterparty Diversity (10%)
  - Sybil Resistance (30%)
  - Evidence Coverage (10%)
- **Multiplicative Penalties**:
  - `if (sybilDetection.is_suspicious) finalTrust = baseTrust * (1 - sybilRisk)`
  - `if (lateFraudDetection.is_suspicious) finalTrust = finalTrust * (1 - lateRisk)`
  - `if (adaptiveAttackerDetection.is_suspicious) finalTrust = finalTrust * (1 - adaptiveRisk)`
  - Multi-vector coordinated attack: If both sybil and adaptive risks >= 0.25 and structural graph collusion exists (circular > 0 or overlap > 0.15), penalty is applied multiplicatively.

---

## 3. Exact Source: `src/evidence.ts`
...
## 4. Exact Source: `src/reputation.ts`
